...
describe("Testing our Application", function () {
    ...
    it("GET After Updating Record /user/:id", () => {
        return request
            .get("v1/user/1")
            .auth(test_user1.username, user_update1.password)
            .then(res => {
                console.log(res.body);
                assert.equal(res.status, 200);
                // assert.equal(res.body.id, "1");
                assert.equal(res.body.username, test_user1.username);
                assert.equal(res.body.first_name, user_update1.first_name);
                assert.equal(res.body.last_name, user_update1.last_name);
...
describe("Testing POST request validations", function () {
    ...
    it("FAIL POST /user with same username", () => {
        return request
            .post("v1/user")
            .send({
                ...test_user1,
    ...
    it("PUT /user/:id with only few f_name & password", () => {
        return (
            request
                ...
                .send({
                    first_name: "updatedfirstname2",
                    password: user_update1.password
...
describe("Testing GET AUTH validations", function () {
    ...
    it("GET /user/:id with wrong username", () => {
        return request
            .get("v1/user/1")
            .auth("username", user_update1.password)
            .then(res => {
                assert.equal(res.status, 401);
            });
    });
